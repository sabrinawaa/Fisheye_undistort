...
def undistort(guess):
    ...
    for i in range(len(guess)):
        ...
        guess[i] = guess[i] * random_number1 / random_number2
    ...
    ##--------# save the undistorted image #--------##
    if not(np.all(result_image == 0)):
        path = r'C:\Users\yx200\Desktop\Fisheye_undistort\chris'
        os.makedirs(path +r'\results', exist_ok=True)
        plt.savefig(path + r'\results\trial_{}.jpg'.format(iterations))
        plt.close()
        with open (path + r'\data.txt'.format(iterations), 'a') as file:
...
# result = minimize(undistort, ini_guess, method='Nelder-Mead')
# minimized_params = result.x
# print(minimized_params)

while True:
    ini_guess = undistort(ini_guess)